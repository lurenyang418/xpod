...
class ArticleFeedParserTest {
  ...
  fun prefersRssFullContentOverEarlierDescription() {
    ...
    val article = parser.parse(feed.toByteArray()).articles.single()
  ...
  @Test
  fun atomSourceIdDoesNotClobberEntryIds() {
    val feed =
        """<feed xmlns="http://www.w3.org/2005/Atom"><title>Atoms</title>
        <entry><id>entry-one</id><title>One</title><source><id>shared-source</id><title>Origin</title></source><content>A</content></entry>
        <entry><id>entry-two</id><title>Two</title><source><id>shared-source</id><title>Origin</title></source><content>B</content></entry>
        </feed>"""

    val result = parser.parse(feed.toByteArray())

    assertEquals(listOf("entry-one", "entry-two"), result.articles.map { it.stableKey })
  }

  @Test
  fun readsRssArtworkFromImageUrlChildElement() {
    val feed =
        """<rss><channel><title>News</title><image><title>News Artwork</title><url>https://example.com/artwork.png</url></image><item><guid>a</guid><title>Hello</title></item></channel></rss>"""

    val result = parser.parse(feed.toByteArray())

    assertEquals("News", result.title)
    assertEquals("https://example.com/artwork.png", result.artworkUrl)
  }

  @Test
  fun rejectsFeedsWithInternalDtdEntityDeclarations() {
    val feed =
        """<?xml version="1.0"?><!DOCTYPE feed [<!ENTITY bomb "boom">]><feed xmlns="http://www.w3.org/2005/Atom"><title>&bomb;</title></feed>"""
