...
class PlaybackService : MediaLibraryService() {
    ...
    override fun onCreate() {
        ...
        val player = ExoPlayer.Builder(this)
            ...
            addListener(object : Player.Listener {
                override fun onEvents(player: Player, events: Player.Events) {
                    if (events.contains(Player.EVENT_POSITION_DISCONTINUITY) || events.contains(Player.EVENT_IS_PLAYING_CHANGED) || events.contains(Player.EVENT_MEDIA_ITEM_TRANSITION) || events.contains(Player.EVENT_PLAYBACK_PARAMETERS_CHANGED)) save(player)
                }
            })
        }
        ...
        session = MediaLibrarySession.Builder(this, player, object : MediaLibrarySession.Callback {
            override fun onGetLibraryRoot(session: MediaLibrarySession, browser: MediaSession.ControllerInfo, params: LibraryParams?): ListenableFuture<LibraryResult<MediaItem>> =
                Futures.immediateFuture(LibraryResult.ofItem(browsableItem("root", "XPOD"), params))

            override fun onGetChildren(session: MediaLibrarySession, browser: MediaSession.ControllerInfo, parentId: String, page: Int, pageSize: Int, params: LibraryParams?): ListenableFuture<LibraryResult<com.google.common.collect.ImmutableList<MediaItem>>> {
                val items = if (parentId == "root") listOf(browsableItem("subscriptions", "Subscriptions"), browsableItem("downloads", "Downloads")) else emptyList()
            ...
            override fun onPlaybackResumption(session: MediaSession, controller: MediaSession.ControllerInfo, isForPlayback: Boolean): ListenableFuture<MediaSession.MediaItemsWithStartPosition> {
                ...
                ioScope.launch {
                    runCatchingCancellable {
                        ...
                        val episodes = playbackRepository.queue().mapNotNull { database.episodes().find(it) }
                        val queue = episodes.ifEmpty { listOf(database.episodes().find(id) ?: error("Previous episode is unavailable")) }
                        val currentIndex = queue.indexOfFirst { it.id == id }.coerceAtLeast(0)
                        MediaSession.MediaItemsWithStartPosition(queue.map { mediaItem(it.id, it.title, it.audioUrl) }, currentIndex, state.positionMs) to state.speed
    ...
    private fun snapshot(player: Player) = PlaybackSnapshot(
    ...
    override fun onDestroy() {
        session?.let { save(it.player); it.player.release(); it.release() }
    ...
    companion object {
        fun component(context: android.content.Context) = ComponentName(context, PlaybackService::class.java)
        private fun browsableItem(id: String, title: String) = MediaItem.Builder().setMediaId(id).setMediaMetadata(MediaMetadata.Builder().setTitle(title).setIsBrowsable(true).build()).build()
        private fun mediaItem(id: String, title: String, url: String) = MediaItem.Builder().setMediaId(id).setUri(url).setMediaMetadata(MediaMetadata.Builder().setTitle(title).setIsPlayable(true).build()).build()
    }

    private data class PlaybackSnapshot(val episodeId: String?, val positionMs: Long, val durationMs: Long, val speed: Float)
}
