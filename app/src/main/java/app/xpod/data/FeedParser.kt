  ...
  private fun parseChannel(parser: XmlPullParser): ParsedFeed {
    ...
    while (parser.next() != XmlPullParser.END_DOCUMENT) {
      ...
      when (parser.name.lowercase()) {
        ...
        "description",
        "subtitle",
        "summary" -> if (description.isBlank()) description = plainText(parser.nextText())
        "author",
  ...
  private fun parseItem(parser: XmlPullParser): ParsedEpisode {
    ...
    while (parser.next() != XmlPullParser.END_DOCUMENT) {
      ...
      when (parser.name.lowercase()) {
        ...
        "description",
        "summary",
        "itunes:summary" -> if (description.isBlank()) description = plainText(parser.nextText())
        ...
        when (parts.size) {
          ...
          3 ->
              parts[0]
                  .toLongOrNull()
                  ?.times(3_600)
                  ?.plus(parts[1].toLongOrNull()?.times(60) ?: return null)
