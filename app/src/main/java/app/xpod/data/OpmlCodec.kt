...
object OpmlCodec {
    fun read(input: InputStream): List<String> {
        ...
        while (parser.next() != XmlPullParser.END_DOCUMENT) {
            if (parser.eventType == XmlPullParser.START_TAG && parser.name.equals("outline", true)) {
                parser.getAttributeValue(null, "xmlUrl")?.trim()?.takeIf { it.startsWith("https://") }?.let(urls::add)
            }
        }
        return urls.toList()
    }
    ...
    fun write(output: OutputStream, podcasts: List<PodcastEntity>) {
        ...
        val content = buildString {
            ...
            podcasts.forEach { append("<outline text=\"").append(escape(it.title)).append("\" type=\"rss\" xmlUrl=\"").append(escape(it.feedUrl)).append("\"/>\n") }
            append("</body></opml>\n")
        }
